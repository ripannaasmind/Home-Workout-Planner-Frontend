...
import {
  Brain, Dumbbell, Timer, Flame, ChevronDown, ChevronUp, Loader2,
  Sparkles, Target, Clock, Zap, Settings, Lock, Activity, Leaf,
  Stethoscope, Lightbulb, Play, Pause, CheckCircle2, Trophy,
  RotateCcw, Check,
} from "lucide-react";
...
const EQUIPMENT_OPTIONS = ["none","dumbbell","barbell","resistance_band","pull_up_bar","bench","kettlebell","yoga_mat"];
const MUSCLE_GROUPS = ["chest","back","shoulders","arms","legs","core","full_body","glutes"];
...
function saveWorkoutLog(log: AIWorkoutLog) {
  try {
    const existing: AIWorkoutLog[] = JSON.parse(localStorage.getItem("ai_workout_logs") || "[]");
    existing.unshift(log);
    localStorage.setItem("ai_workout_logs", JSON.stringify(existing.slice(0, 50)));
  } catch { /* ignore */ }
}
...
export default function AIWorkoutPage() {
  ...
  const startTimer = () => {
    setElapsed(0); setDoneExercises(new Set());
    setTimerActive(true); setTimerPaused(false);
  };
  const pauseTimer = () => setTimerPaused((p) => !p);
  const stopWorkout = () => {
    setTimerActive(false); setTimerPaused(false);
    if (timerRef.current) clearInterval(timerRef.current);
    setShowFinish(true);
  };
  const resetWorkout = () => {
    setTimerActive(false); setTimerPaused(false);
    setElapsed(0); setDoneExercises(new Set()); setShowFinish(false);
  };
  ...
  const handleFinishConfirm = () => {
    if (!result) return;
    saveWorkoutLog({
      id: `${Date.now()}`, name: result.name, description: result.description,
      difficulty: result.difficulty, category: result.category,
      exerciseCount: result.exercises?.length || 0,
      estimatedCalories: result.estimatedCalories || 0,
      actualDuration: elapsed, completedAt: new Date().toISOString(), goal,
    });
    setShowFinish(false); resetWorkout(); setResult(null);
  };

  const toggleEquipment = (item: string) =>
    setEquipment((prev) => prev.includes(item) ? prev.filter((e) => e !== item) : [...prev, item]);
  const toggleMuscle = (item: string) =>
    setTargetMuscles((prev) => prev.includes(item) ? prev.filter((m) => m !== item) : [...prev, item]);

  const handleGenerate = async () => {
    if (!goal || !fitnessLevel || !duration) { setError("Please fill in all required fields"); return; }
    if (!token) return;
    setLoading(true); setError(""); setResult(null); resetWorkout();
    try {
      const res = await aiApi.generateWorkout(
        { goal, fitnessLevel, duration,
      ...
      else { setError("Workout generated but result was empty. Please try again."); }
    } catch (err) {
      const raw = err instanceof Error ? err.message : "Failed to generate workout";
      let msg = raw;
      try { const p = JSON.parse(raw); msg = p?.error?.message || p?.message || raw; } catch { /* not JSON */ }
  ...
  return (
    ...
    <div className="space-y-6 max-w-4xl mx-auto page-fade">
      ...
      {aiStatus === null && (
        <Card><CardContent className="flex items-center justify-center py-12 gap-3 text-muted-foreground">
          <Loader2 className="h-5 w-5 animate-spin" /><span>Checking AI availability…</span>
        </CardContent></Card>
      )}
        ...
        <Card className="border-dashed border-2 border-orange-300 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-800">
          <CardContent className="flex flex-col items-center gap-4 py-12 text-center">
            ...
            <div>
              ...
              <p className="text-muted-foreground max-w-md">An administrator needs to add the AI API key and enable it from the admin settings.</p>
            </div>
            {(user as { role?: string })?.role === "admin" && (
              <Button asChild className="gap-2"><Link href="/admin/settings"><Settings className="h-4 w-4" />Go to Admin Settings</Link></Button>
            )}
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg">
            <Sparkles className="h-5 w-5 text-primary" />Customize Your Workout
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {GOALS.map((g) => (
                <button key={g.value} onClick={() => setGoal(g.value)}
                  className={`flex items-center gap-2 p-3 rounded-lg border text-sm font-medium transition-all ${goal === g.value ? "border-primary bg-primary/5 dark:bg-primary/10 text-primary-dark dark:text-primary" : "border-gray-200 dark:border-gray-700 hover:border-primary/50"}`}>
            ...
            <div className="flex gap-2">
              {FITNESS_LEVELS.map((level) => (
                <button key={level} onClick={() => setFitnessLevel(level)}
                  className={`flex-1 py-2.5 px-4 rounded-lg border text-sm font-medium capitalize transition-all ${fitnessLevel === level ? "border-primary bg-primary/5 dark:bg-primary/10 text-primary-dark dark:text-primary" : "border-gray-200 dark:border-gray-700 hover:border-primary/50"}`}>
                  {level}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3">
              {[15, 20, 30, 45, 60].map((d) => (
                <button key={d} onClick={() => setDuration(d)}
                  className={`py-2 px-4 rounded-lg border text-sm font-medium transition-all ${duration === d ? "border-primary bg-primary/5 dark:bg-primary/10 text-primary-dark dark:text-primary" : "border-gray-200 dark:border-gray-700 hover:border-primary/50"}`}>
                  {d}m
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {EQUIPMENT_OPTIONS.map((eq) => (
                <button key={eq} onClick={() => toggleEquipment(eq)}
                  className={`py-1.5 px-3 rounded-full border text-xs font-medium capitalize transition-all ${equipment.includes(eq) ? "border-primary bg-primary/10 dark:bg-primary/20 text-primary-dark dark:text-primary" : "border-gray-200 dark:border-gray-700 hover:border-primary/50"}`}>
                  {eq.replace(/_/g, " ")}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {MUSCLE_GROUPS.map((muscle) => (
                <button key={muscle} onClick={() => toggleMuscle(muscle)}
                  className={`py-1.5 px-3 rounded-full border text-xs font-medium capitalize transition-all ${targetMuscles.includes(muscle) ? "border-primary bg-primary/10 dark:bg-primary/20 text-primary-dark dark:text-primary" : "border-gray-200 dark:border-gray-700 hover:border-primary/50"}`}>
                  {muscle.replace(/_/g, " ")}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-2">
            ...
            <Input placeholder="e.g., Low impact, no jumping, focus on posture..." value={preferences} onChange={(e) => setPreferences(e.target.value)} />
          </div>
          {error && (
          ...
          <Button onClick={handleGenerate} disabled={loading || !goal || !fitnessLevel} className="w-full bg-primary hover:bg-primary-dark text-white" size="lg">
            {loading ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Generating… This may take up to 30 seconds</>) : (<><Sparkles className="h-4 w-4 mr-2" />Generate AI Workout</>)}
          </Button>
        </CardContent>
      </Card>
      ...
      {result && (
        <div ref={resultRef} className="space-y-4">
          <Card className="border-primary/20 dark:border-primary/30">
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between flex-wrap gap-2">
                ...
                <Badge variant="outline" className="capitalize shrink-0">{result.difficulty}</Badge>
              </div>
              <div className="flex flex-wrap gap-3 mt-3">
                <div className="flex items-center gap-1 text-sm text-muted-foreground"><Clock className="h-4 w-4" />{result.estimatedDuration} min</div>
                <div className="flex items-center gap-1 text-sm text-muted-foreground"><Flame className="h-4 w-4 text-orange-500" />~{result.estimatedCalories} cal</div>
                <div className="flex items-center gap-1 text-sm text-muted-foreground"><Target className="h-4 w-4 text-blue-500" />{result.category?.replace(/_/g, " ")}</div>
                <div className="flex items-center gap-1 text-sm text-muted-foreground"><Zap className="h-4 w-4 text-yellow-500" />{result.exercises?.length} exercises</div>
              </div>
            </CardHeader>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col sm:flex-row items-center gap-4">
                ...
                {timerActive && (
                  <div className="flex-1 w-full">
                    <div className="flex justify-between text-xs text-muted-foreground mb-1">
                      <span>Exercise Progress</span><span>{completedCount}/{totalExercises} done</span>
                    </div>
                ...
                <div className="flex gap-2 shrink-0">
                  {!timerActive ? (
                    <Button onClick={startTimer} className="bg-primary hover:bg-primary-dark text-white gap-2">
                    ...
                    <>
                      ...
                      <Button onClick={stopWorkout} className="bg-green-600 hover:bg-green-700 text-white gap-2">
              ...
              {timerPaused && <p className="text-center text-xs text-amber-500 font-medium mt-2">⏸ Workout paused — take a breather!</p>}
            </CardContent>
          </Card>
                ...
                <CardContent className="pt-0 space-y-2">
                  {result.warmup.exercises?.map((ex, i) => (
                    <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                      ...
                      <div><p className="font-medium text-sm">{ex.name}</p><p className="text-xs text-muted-foreground">{ex.duration}s • {ex.instructions}</p></div>
                    </div>
                  ))}
                </CardContent>
              )}
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-lg"><Dumbbell className="h-5 w-5 text-primary" />Exercises</CardTitle>
                {timerActive && <span className="text-xs text-muted-foreground font-medium">Tap exercise to mark done ✓</span>}
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              {result.exercises?.map((ex, i) => {
                ...
                return (
                  <div key={i} onClick={() => timerActive && toggleExercise(i)}
                    className={`p-4 rounded-lg border transition-all ${timerActive ? "cursor-pointer select-none" : ""} ${done ? "border-green-300 bg-green-50 dark:bg-green-900/20 dark:border-green-700" : "border-gray-100 dark:border-gray-800 hover:border-primary/30"}`}>
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-3">
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${done ? "bg-green-500" : "bg-primary/10 dark:bg-primary/20"}`}>
                          {done ? <Check className="h-4 w-4 text-white" /> : <span className="text-sm font-bold text-primary-dark">{i + 1}</span>}
                        </div>
                        <div>
                          <p className={`font-semibold text-sm ${done ? "line-through text-muted-foreground" : ""}`}>{ex.name}</p>
                          <p className="text-xs text-muted-foreground capitalize">{ex.muscleGroup}</p>
                        </div>
                      </div>
                      <div className="flex gap-1.5 flex-wrap justify-end text-xs">
                        {ex.sets > 0 && <Badge className="bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 border-0 font-semibold">{ex.sets} sets</Badge>}
                        {ex.reps > 0 && <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300 border-0 font-semibold">{ex.reps} reps</Badge>}
                    ...
                    {!done && <>
                      ...
                      {ex.restTime > 0 && <p className="text-xs text-muted-foreground mt-1">Rest: {ex.restTime}s</p>}
                    </>}
                    {done && <p className="text-xs text-green-600 font-medium mt-1 flex items-center gap-1"><CheckCircle2 className="h-3 w-3" />Completed!</p>}
                  </div>
                );
              })}
            </CardContent>
          ...
          {result.cooldown && (
            <Card>
              <button onClick={() => setShowCooldown(!showCooldown)} className="w-full flex items-center justify-between p-4">
                <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={() => { resetWorkout(); setResult(null); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="w-full gap-2">
            <RotateCcw className="h-4 w-4" />Generate Another Workout
          </Button>
        </div>
      )}
      </>)}
