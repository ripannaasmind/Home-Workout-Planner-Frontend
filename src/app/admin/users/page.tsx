    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={roleFilter} onValueChange={setRoleFilter}>
          <SelectTrigger className="w-36 bg-white dark:bg-card border-gray-200 dark:border-gray-800">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                  ...
                  <th className="text-left px-4 py-3 text-gray-500 dark:text-gray-400 font-medium">Joined</th>
                  <th className="text-right px-4 py-3 text-gray-500 dark:text-gray-400 font-medium">Actions</th>
                </tr>
              </thead>
              <tbody>
                {users.map((u) => (
                  <tr key={u._id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:bg-gray-800/50 transition-colors">
                    <td className="px-4 py-3 font-medium text-gray-800 dark:text-gray-100">{u.name}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{u.email}</td>
                    <td className="px-4 py-3">
                      <Badge className={u.role === "admin" ? "bg-primary/10 text-primary border-0" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-0"}>
                        {u.role}
                      </Badge>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          size="sm"
                          variant="ghost"
                          className="h-8 w-8 p-0 rounded-lg border border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400 hover:bg-primary hover:text-white hover:border-primary transition-colors"
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          className="h-8 w-8 p-0 rounded-lg border border-gray-200 dark:border-gray-800 text-red-500 hover:bg-red-50 dark:hover:bg-red-50 dark:hover:bg-red-500/100/100 hover:text-white hover:border-red-500 transition-colors"
      ...
      <Dialog open={!!editUser} onOpenChange={() => setEditUser(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div>
              <Label className="text-gray-700 dark:text-gray-200 text-sm mb-1 block">Name</Label>
              <p className="text-gray-800 dark:text-gray-100 font-medium">{editUser?.name}</p>
            </div>
            <div>
              <Label className="text-gray-700 dark:text-gray-200 text-sm mb-1 block">Email</Label>
              <p className="text-gray-600 dark:text-gray-300 text-sm">{editUser?.email}</p>
            </div>
            <div>
              <Label className="text-gray-700 dark:text-gray-200 text-sm mb-1.5 block">Role</Label>
